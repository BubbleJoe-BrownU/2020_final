...
wave_2_filtered <- data_wave_2[, c("idnum", "cf2md_case_con", "f2g5", "f2g5a", "cf2age", "cm2fbir", "cf2edu", 
                          "cm2edu", "f2g7b", "f2h7a", "m2c33", "f2l1", "cm2md_case_con", "cm2md_case_lib", "cf2marm", 
                          "cf2cohm", "f2b1", "f2b1a", "f2b2", "f2b3")]
wave_1_filtered <- data_wave_1[, c("idnum", "m1h3", "m1h3a")]

combined_data <- left_join(wave_2_filtered, wave_1_filtered, by = "idnum")

#exclue -9 in cf2md_case_con
combined_data <- combined_data %>%
  filter(cf2md_case_con != -9 & cf2md_case_con != -3)
head(combined_data)
```
```{r}
...
```{r}
race_labels <- c( "-6 Skip", "-5 Not asked", "-3 Missing", "-2 Don't know", "-1 Refuse", "1 White", "2 Blck", "3 Asian", "4 AmInd", "5 Other", "101 Hispanic") 
combined_data$f2g5_factor <- factor(combined_data$f2g5, labels = race_labels)

f2g5_factor_frequencies <- table(combined_data$f2g5_factor)
