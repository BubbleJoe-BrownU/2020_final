...
combined_race_count <- combined_data_filtered %>%
  group_by(f1h3_factor, cf2md_case_lib) %>%
  summarize(race_count = n(), .groups = 'drop')  # Ensure .groups='drop' to avoid group data frame output

colors <- c(...)  # Ensure this variable is defined with appropriate color values
...
ggplot(combined_race_count, aes(x = reorder(f1h3_factor, race_count), y = race_count, fill = cf2md_case_lib)) +
  ...
  labs(title = "Distribution of Father's Race", 
       ...
       y = "Frequency") +
  scale_fill_manual(values = colors, name = "Depression") +
  scale_y_continuous(breaks = seq(0, max(combined_race_count$race_count), by = 10)) + # Adjust breaks as needed
  theme_minimal() +
  theme(
    panel.grid.minor = element_blank(), 
    panel.grid.major.x = element_blank(),
